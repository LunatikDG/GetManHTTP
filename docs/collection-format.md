...
| `JwtBearer` | коллекция, папка, запрос | `algorithm`, `secret`, `privateKey`, `claims`, `ttlSeconds` | Собирает JWT через платформенный `ТокенДоступа` и шлёт `Authorization: Bearer {jwt}`. Алгоритмы: **HS256/HS384/HS512** (материал: `secret`) и **RS256/RS384/RS512** (материал: `privateKey` PEM без пароля, `BEGIN PRIVATE KEY` / `BEGIN RSA PRIVATE KEY`). `algorithm` по умолчанию `HS256`. `claims` — JSON-объект строкой; `ttlSeconds` &gt; 0 задаёт `iat`/`exp` (Unix). И `secret`, и `privateKey` — секреты; пустой нужный материал — без заголовка Authorization. |
